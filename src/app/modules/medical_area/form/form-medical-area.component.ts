...
@Component({
    selector: 'app-form-medical-area',
    templateUrl: './form-medical-area.component.html',
...
export class FormMedicalAreaComponent implements OnInit {
    ...
    private medicalAreaService: MedicalAreaService = inject(MedicalAreaService);
    _id: string | undefined;
    @Input() set id(value: string | undefined) {
        if (value) {
            this._id = value;
            this.medicalAreaService.getById(this.id).subscribe({
                next: (data) => {
                    if (data) {
                        this.form.patchValue(data);
                    } else {
                        this.messageService.add({ severity: 'error', key: 'msg', summary: 'No se pudo encontrar el área médica', life: 3000 });
                    }
                },
                error: (error) => {
