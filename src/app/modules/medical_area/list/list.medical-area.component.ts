...
@Component({
    selector: 'app-list-medical-area',
    standalone: true,
    imports: [
        CommonModule,
        ButtonModule,
    ...
    providers: [MedicalAreaService, MessageService, ConfirmationService],
    templateUrl: './list.medical-area.component.html'
})
export class ListMedicalAreaComponent {
    columns = [
        {
            field: 'id',
            column: 'ID',
    ...
    listTable(): void {
        ...
        this.medicalAreaService.getList(this.limit, this.page, this.sort, this.search).subscribe({
            next: (data) => {
                if (data['meta']['totalItems']) {
                    this.data = data['data'];
                    this.totalRows = data['meta']['totalItems'];
                    this.configTable = {
                        module: 'Áreas médicas',
                        route: 'medical_area',
                        totalRows: this.totalRows
                    };
                } else {
                    this.messageService.add({ life: 5000, key: 'message', severity: 'error', summary: 'Error', detail: 'No se encontraron áreas médicas' });
                    this.data = [];
                    this.totalRows = 0;
                    this.configTable = {
                        module: 'Áreas médicas',
            ...
            error: (error) => {
                this.miscsService.endRquest();
                this.data = [];
                this.messageService.add({ life: 5000, key: 'message', severity: 'error', summary: 'Error cargando la lista de áreas médicas', detail: error.error.message });
            }
        });
    }
            ...
            accept: () => {
                switch (deleteType) {
                    case 1:
                        ...
                        break;
                    case 2:
                        this.medicalAreaService.disable(id).subscribe(
                            () => {
                                this.listTable();
                                this.messageService.add({ severity: 'success', key: 'message', summary: 'Operación exitosa', life: 3000 });
                            },
                            (error) => {
                                this.messageService.add({ life: 5000, key: 'message', severity: 'error', summary: 'Error al eliminar el área médica', detail: error.error.message });
                            }
                        );
                        break;
                }
    ...
    deleteSelected() {
        ...
        for (let i = 0; i < this.ids.length; i++) {
            const ptt = this.medicalAreaService.disable(this.ids[i]).pipe(
                catchError((error) => {
