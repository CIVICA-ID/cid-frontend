...
interface CellStaysListResponse {
  data?: CellStay[];
  meta?: {
    totalItems?: number;
  };
...
export class ListComponent implements OnInit {
  ...
  readonly workflowStage = getWorkflowStage('cell-stays');
  ...
  constructor(
    private readonly cellStaysService: CellStaysService,
  ...
  ngOnInit(): void {
    this.reloadList$
      .pipe(
        ...
        switchMap(() =>
          this.cellStaysService.getList(this.limit, this.page, this.sort, this.search).pipe(
            catchError((error: unknown) => {
              this.resetList();
              this.showErrorMessage('Error cargando la lista de estadías en celda', error);
              return of<CellStaysListResponse | null>(null);
            }),
  ...
  private handleListResponse(response: CellStaysListResponse | null): void {
    if (!response) {
      return;
    }
    ...
    this.messageService.add({
      ...
      key: 'message',
      severity: 'warn',
      summary: 'No se encontraron estadías en celda',
      detail: ''
    });
    this.resetList();
  }
  ...
  private deleteRecords(idsToDelete: string[], deleteType: DeleteType): void {
    ...
    const requests = idsToDelete.map((itemId) =>
      this.cellStaysService.disable(itemId).pipe(
        catchError((error: unknown) => {
          this.messageService.add({
            life: 5000,
            key: 'message',
